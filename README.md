...
对照版式（「原文 + 翻译 + 空耳」同框）已进入正式流水线，作者选定 **A2 / B1 / C2 / C4 / E3** 五套：

```bash
python make_ass.py --layout=E3            # 生成 soramimi_ice_rainbow_E3.ass
python burn.py ice_rainbow --layout=E3 --trim=2.85 --preview=90
python make_ass.py --selftest             # 断言不传 --layout 时输出没变
```

| 版式 | 内容 | 画面 |
|---|---|---|
| `A2` | 中译 / 日文 / 空耳，三行叠压 | 不缩 |
| `B1` | 画面居左 940×529，右栏三行 | 24% 面积 |
| `C2` | 底部黑带放空耳+中译，日文浮顶 | 缩到 75% |
| `C4` | 半透明色带，画面不缩 | 不缩 |
| `E3` | 逐字注音 + 空耳 + 日文顶 + 中译底 | 不缩 |

**不传 `--layout` 时 `make_ass.py` 的输出与加这个功能之前逐字节相同** ——
`--selftest` 断言事件层只有原来的 layer 0/1、边距字段全为 0、样式只有 `Sora`；
另外用 `cmp` 与作者已确认满意的那份 `.ass` 比对过，`IDENTICAL`。

`--preview=90` 只压前 90 秒。它用的是 `-t`（限制输出时长）而不是 `-ss`，起点仍是 0、
时间戳不重置，所以 §6.6 那类字幕整体偏移**不会**发生——危险的是「从中间截取」，
不是「只压前 N 秒」。

选型阶段用的探索脚本（保留为记录，排版实现已全部并回 `make_ass`）：
